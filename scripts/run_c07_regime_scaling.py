"""Generate immutable C07 regime-aware scaling artifacts."""
...
from calibrated_reliability.experiments.c07 import C07Config, run_c07_seed
...
@click.option("--output-root", type=click.Path(path_type=Path), default=Path("outputs/c07"))
def main(config_path: Path, registry_path: Path, data_root: Path, output_root: Path) -> None:
    """Fit once per seed on FD001 and evaluate the three declared C07 targets."""
    sha = subprocess.check_output(["git", "rev-parse", "HEAD"], text=True).strip()
    if subprocess.check_output(["git", "status", "--porcelain"], text=True).strip():
        raise click.ClickException("C07 requires a clean Git worktree")
    config = C07Config.from_yaml(config_path.read_text(encoding="utf-8"))
    ...
    for seed in config.seeds:
        results = run_c07_seed(train, target_data, config, seed)
