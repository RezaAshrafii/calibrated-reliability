...
C08 is an adaptive conformal prequential evaluation. It fits and calibrates the C02 FD001 pipeline once per seed, then evaluates target endpoints in increasing official engine-ID order. Each interval uses only the current alpha and fixed FD001 calibration residuals; after the endpoint outcome is revealed, its miss indicator updates alpha for the next endpoint. Gamma is fixed at 0.01 and alpha is projected to [0.001, 0.999]. C08 does not use target outcomes for fitting, tuning, initial calibration, or a current interval. It reports a sequential simulation rather than a batch target-domain recalibration or a universal shift-coverage guarantee.
